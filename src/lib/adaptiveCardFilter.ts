...
function getBlocks(
  cardCollection: AdaptiveCardElement | AdaptiveCardElement[],
  types: CARD_TYPES | CARD_TYPES[]
): AdaptiveCardElement[] {
  const typesArray: CARD_TYPES[] = toArray(types);
  const cardCollectionArray: AdaptiveCardElement[] = toArray(cardCollection);
  return cardCollectionArray.filter((card) =>
    typesArray.some((type) => isCardType(card, type))
  );
}
...
export function getTextBlocks(
  cardCollection: AdaptiveCardElement[]
): ITextBlock[] {
  return getBlocks(cardCollection, CARD_TYPES.TEXT_BLOCK) as ITextBlock[];
}

export function getNonTextBlocks(
  cardCollection: AdaptiveCardElement[]
): Array<NonTextAdaptiveCardElement> {
  return getBlocks(cardCollection, [
    CARD_TYPES.IMAGE,
    CARD_TYPES.CONTAINER,
  ]) as Array<NonTextAdaptiveCardElement>;
}

export function getTextBlocksAsString(
  cardCollection: AdaptiveCardElement[]
): string {
